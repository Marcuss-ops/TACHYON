...
public:
    ...
    static void split_cubic_bezier(const PathVertex& v1, const PathVertex& v2, double t, PathVertex& left_end, PathVertex& right_start) {
        ...
        // Adjust the outgoing tangent of the original start and incoming tangent of the original end
        // This is a bit complex for a single function, so we'll handle it in trim_paths.
    }
    ...
    static ShapePath trim_paths(const ShapePath& path, double start, double end, double offset) {
        ...
        for (const auto& subpath : path.subpaths) {
            ...
            // 1. Calculate arc lengths per segment (using 10-step approximation)
            ...
            auto extract_range = [&](double t_start, double t_end) {
                ...
                for (size_t i = 0; i < num_segs; ++i) {
                    ...
                    if (local_e > local_s) {
                        ...
                        // Apply De Casteljau to get the sub-segment
                        if (local_e < 1.0) {
                            PathVertex dummy;
                        ...
                        if (sub.vertices.empty()) {
                            ...
                            // Merge with previous vertex if they are basically the same point
        ...
        auto flatten_subpath = [](const ShapeSubpath& sub, PathsD& paths) {
            ...
            for (size_t i = 0; i < sub.vertices.size(); ++i) {
                ...
                if (v1.out_tangent.x == 0 && v1.out_tangent.y == 0 && v2.in_tangent.x == 0 && v2.in_tangent.y == 0) {
                    ...
                    // Recursive flattening or simple high-sample flattening for this milestone
                    const int steps = 24;

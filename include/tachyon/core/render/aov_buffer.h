...
/**
 * @brief Represents the multi-pass output of a renderer (2D or 3D).
 * Moved from renderer3d to core to allow decoupling.
 */
struct AOVBuffer {
    ...
    std::vector<float> albedo_rgb;       // Diffuse albedo (3 channels)
    ...
    void clear() {
        ...
        std::fill(albedo_rgb.begin(), albedo_rgb.end(), 0.0f);
    ...
    void allocate(std::uint32_t w, std::uint32_t h) {
        ...
        albedo_rgb.assign(num_pixels * 3, 0.0f);

...
public:
    static TextLayoutResult layout(
        ...
        if (layout_cache.get(layout_cache_key, result)) return result;
        ...
        for (const auto& run : bidi_runs) {
            ...
            for (std::size_t i = 0; i < run.length; ) {
                ...
                for (const auto& c : clusters) {
                    if (cp_index >= c.start_index && cp_index < c.start_index + c.length) { current_cluster = &c; break; }
                }
                ...
                for (const auto& f : fallback_chain) {
                    ...
                    for (std::size_t k = 0; k < current_cluster->length; ++k) {
                        ...
                        if (!(f->has_glyph(cp) || cp == '\n' || cp == ' ' || cp == '\t' || cp == 0x200D)) { all_supported = false; break; }
                    }
                    if (all_supported) { selected_font = f; break; }
                }
                ...
                if (cluster_end_index > run.start) i = cluster_end_index - run.start; else i++;
            }
...
#pragma omp parallel for
            for (int idx = 0; idx < (int)subruns.size(); ++idx) {
                ...
                cache_key.font_id = sub.font->id(); cache_key.scale = scale; cache_key.direction = dir;
                cache_key.script = script_info.script; cache_key.language = script_info.language;
                cache_key.codepoints = sub.codepoints; cache_key.features = style.features;
            ...
            for (size_t idx = 0; idx < subruns.size(); ++idx) {
                const auto& sub = subruns[idx]; const auto& shaped = shaped_runs[idx];
                bool contains_break = false;
                for (std::uint32_t cp : sub.codepoints) {
                    if (is_breakable_space(cp) || cp == '-' || (cp >= 0x4E00 && cp <= 0x9FFF)) { contains_break = true; break; }
                }
                if (wrap_width > 0 && text_box.multiline && options.word_wrap && pen_x > 0 && std::abs(shaped.width) > 0 && (pen_x + std::abs(shaped.width)) > wrap_width) {
                    ...
                    pen_x = 0; current_line_width = 0; pen_y += scaled_line_height; line_start = result.glyphs.size(); last_was_space = true;
                }
...
TextLayoutResult layout_text(const BitmapFont& font, std::string_view utf8_text, const TextStyle& style, const TextBox& text_box, TextAlignment alignment, const TextLayoutOptions& options) {
    ...
    std::vector<const Font*> chain = { &font }; return InternalLayoutEngine::layout(chain, utf8_text, style, text_box, alignment, options);
}

TextLayoutResult layout_text(const FontRegistry& registry, const std::string& font_name, std::string_view utf8_text, const TextStyle& style, const TextBox& text_box, TextAlignment alignment, const TextLayoutOptions& options) {
    ...
    if (fallback_chain.empty()) {
        ...
        if (default_font) fallback_chain = {default_font}; else return {};
    }

...
const char* report_status(const DiagnosticBag& diagnostics) {
    if (!diagnostics.ok()) {
        return REPORT_STATUS_ERROR;
    }
    if (diagnostics.has_warnings()) {
        return REPORT_STATUS_WARNING;
    }
    return REPORT_STATUS_OK;
}

void apply_schema(json& report, const char* schema_name, const char* schema_version) {
...
std::string make_version_report_json(const DiagnosticBag& diagnostics) {
    json report;
    report["report_type"] = "version";
    apply_schema(report, VERSION_REPORT_SCHEMA_NAME, VERSION_REPORT_SCHEMA_VERSION);
    report["version"] = version_string();
    report["status"] = report_status(diagnostics);
...
std::string make_inspect_report_json(
    ...
    apply_schema(report, INSPECT_REPORT_SCHEMA_NAME, INSPECT_REPORT_SCHEMA_VERSION);
...
std::string make_validate_report_json(
    ...
    apply_schema(report, VALIDATE_REPORT_SCHEMA_NAME, VALIDATE_REPORT_SCHEMA_VERSION);
    report["scene"] = make_scene_json(scene);
...
std::string make_render_report_json(
    ...
    apply_schema(report, RENDER_REPORT_SCHEMA_NAME, RENDER_REPORT_SCHEMA_VERSION);
    report["scene"] = make_scene_json(scene);
    report["assets"] = make_assets_json(assets);
    report["status"] = report_status(diagnostics);
    report["diagnostics"] = make_diagnostics_json(diagnostics);

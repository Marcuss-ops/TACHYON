...
bool parse_tool_option(const std::string& arg, const std::vector<std::string>& args, std::size_t& index, CliOptions& options, DiagnosticBag& diagnostics) {
    ...
    if (arg == "--inputs") {
        ...
        if (val.empty()) diagnostics.add_error("cli.inputs_missing", "missing value for --inputs");
            ...
            while (end != std::string::npos) {
                options.media.concat_inputs.push_back(val.substr(start, end - start));
                start = end + 1;
                end = val.find(',', start);
            }
            options.media.concat_inputs.push_back(val.substr(start));
        }
        return true;
    }
    ...
    if (arg == "--out" && options.command == "concat") {
        options.media.concat_output = require_argument(args, index);
        return true;
    }

    // Font options
    if (arg == "--family") { options.tools.font_family = require_argument(args, index); return true; }
    if (arg == "--dest") { options.tools.font_dest = require_argument(args, index); return true; }
    if (arg == "--overwrite") { options.tools.font_overwrite = true; return true; }
    
    // Thumb options
    if (arg == "--frame" && options.command == "thumb") {
        std::string val = require_argument(args, index);
        try { options.tools.thumb_frame = std::stoi(val); }

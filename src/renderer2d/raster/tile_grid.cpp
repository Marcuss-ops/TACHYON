...
TileGrid build_tile_grid_from_roi(const RectI& roi, std::int64_t frame_width, std::int64_t frame_height, int tile_size) {
    TileGrid grid;
    grid.tile_size = std::max(1, tile_size);
    grid.roi = clamp_to_frame(roi, frame_width, frame_height);
    if (grid.roi.width <= 0 || grid.roi.height <= 0) return grid;
    ...
    for (int y = grid.roi.y; y < grid.roi.y + grid.roi.height; y += grid.tile_size) {
        for (int x = grid.roi.x; x < grid.roi.x + grid.roi.width; x += grid.tile_size) {
            ...
            grid.tiles.push_back(RectI{x, y, tile_width, tile_height});
        }
    }
    return grid;
}
...
TileGrid build_tile_grid(const RectI& roi, std::int64_t frame_width, std::int64_t frame_height, int tile_size) {

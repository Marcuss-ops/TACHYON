...
#include "tachyon/renderer2d/raster/perspective_edge.h"
...
using Edge = PerspectiveEdge;

void draw_scanline(
    SurfaceRGBA& surface,
    int y,
    const PerspectiveEdge& left,
    const PerspectiveEdge& right,
    ...
    for (int x = x_start; x < x_end; ++x) {
        ...
        if (surface.test_and_write_depth(static_cast<std::uint32_t>(x), static_cast<std::uint32_t>(y), one_w)) {
            ...
            // Wrap or clamp UV depending on the sampler configuration.
...
void WarpRasterizer::draw_triangle(
    SurfaceRGBA& surface,
    const WarpVertex& v0_in,
    const WarpVertex& v1_in,
    const WarpVertex& v2_in,
    const SurfaceRGBA* texture,
    float opacity) {

    if (!texture) return;

    const WarpVertex* pts[3] = {&v0_in, &v1_in, &v2_in};
...
void WarpRasterizer::draw_quad(SurfaceRGBA& surface, const PerspectiveWarpQuad& quad) {

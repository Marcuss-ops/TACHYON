...
std::shared_ptr<SurfaceRGBA> render_simple_layer_surface(
    ...
    if (layer.type == scene::LayerType::Text) {
        ...
        if (!layer.font_id.empty()) {
            font = registry->find(layer.font_id);
        }
        if (font == nullptr) {
        ...
        if (!layout.glyphs.empty()) {
            ...
            for (const auto& highlight : layer.text_highlights) {
                if (highlight.end_glyph <= highlight.start_glyph || highlight.start_glyph >= layout.glyphs.size()) {
                    continue;
                }
                ...
                for (std::size_t i = highlight.start_glyph; i < end; ++i) {
                    ...
                    max_x = std::max(max_x, g.x + g.width);
                    max_y = std::max(max_y, g.y + g.height);
                }

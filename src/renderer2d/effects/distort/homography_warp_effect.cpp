...
registry::ParameterSchema HomographyWarpEffect::schema() {
    registry::ParameterSchema s;
    for (int i = 0; i < 9; ++i) {
        std::string key = "h" + std::to_string(i);
        s.add_scalar(key, "Matrix element " + std::to_string(i), (i % 4 == 0) ? 1.0f : 0.0f);
    }
    return s;
}

void HomographyWarpEffect::render(
    ...
    const EffectParams& params) 
{
    ...
    for (std::uint32_t y = 0; y < output.height(); ++y) {
        for (std::uint32_t x = 0; x < output.width(); ++x) {
            float nx = (x + 0.5f) / output.width();
            float ny = (y + 0.5f) / output.height();
            ...
            src_x = (src_x * output.width()) - 0.5f;
            src_y = (src_y * output.height()) - 0.5f;
...
SurfaceRGBA HomographyWarpEffect::apply(const SurfaceRGBA& input, const EffectParams& params) const {
    SurfaceRGBA out;
    render(EffectSpec{}, input, out, {}, params);
    return out;
}

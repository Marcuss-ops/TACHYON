#include "tachyon/runtime/media/streaming/media_prefetcher.h"
...
void MediaPrefetcher::update(IMediaProvider& manager, IPlaybackScheduler& scheduler, const std::vector<std::string>& active_video_paths, double current_time, double fps, int prefetch_count) {
    if (fps <= 0.0) return;
    ...
    for (const auto& path : active_video_paths) {
        ...
        const double duration = manager.get_duration(path);
        ...
        for (int i = 1; i <= prefetch_count; ++i) {
            ...
            if (!already_requested) {
                ...
                scheduler.schedule_task({path, prefetch_time, IPlaybackScheduler::Priority::Prefetch});
            }
        }
    }
}

...
const renderer2d::SurfaceRGBA* MediaManager::get_image(
    ...
    std::filesystem::path resolved = resolve_media_path_detailed(path, ResolutionPurpose::Playback);
...
double MediaManager::get_duration(const std::filesystem::path& path) {
    std::filesystem::path resolved = resolve_media_path(path);
...
const renderer2d::SurfaceRGBA* MediaManager::get_video_frame(const std::filesystem::path& path, double time, ::tachyon::DiagnosticBag* diagnostics) {
    std::filesystem::path resolved = resolve_media_path_detailed(path, ResolutionPurpose::Playback);
...
std::filesystem::path MediaManager::resolve_media_path_detailed(
    const std::filesystem::path& path,
...
void MediaManager::store_video_frame(const std::filesystem::path& path, double time, std::unique_ptr<renderer2d::SurfaceRGBA> frame) {
    const std::filesystem::path resolved = resolve_media_path_detailed(path, ResolutionPurpose::Playback);
    const std::string key = make_video_frame_cache_key(resolved, time);
    m_frame_cache->put(key, std::move(frame));
}
...
::tachyon::DiagnosticBag MediaManager::consume_diagnostics() {
    std::lock_guard<std::mutex> lock(m_mutex);

...
void BackgroundKindRegistry::load_builtins() {
    ...
    auto get_procedural_params = [](const ParameterBag& bag) {
        ProceduralParams p;
        p.palette_a = bag.get_or<ColorSpec>("palette.a", {0,0,0,255});
        p.palette_b = bag.get_or<ColorSpec>("palette.b", {255,255,255,255});
        p.palette_c = bag.get_or<ColorSpec>("palette.c", {0,0,0,0});
    ...
    register_spec({
        ...
        ParameterSchema({
            {"motion_speed", "Speed", "Animation speed", 1.0, 0.0, 10.0},
            {"contrast", "Contrast", "Visual contrast", 1.0, 0.1, 5.0},
            {"softness", "Softness", "Edge softness", 0.0, 0.0, 1.0},
            {"grain", "Grain", "Film grain intensity", 0.0, 0.0, 1.0},
            {"palette.a", "Color A", "Primary color", ColorSpec{10, 10, 20, 255}},
            {"palette.b", "Color B", "Secondary color", ColorSpec{50, 20, 80, 255}},
    ...
    register_spec({
        ...
        ParameterSchema({
            {"spacing", "Spacing", "Grid cell size", 60.0, 10.0, 500.0},
            {"motion_speed", "Speed", "Scroll speed", 1.0, 0.0, 10.0},
            {"palette.a", "Background", "Grid background color", ColorSpec{5, 5, 10, 255}},
            {"palette.b", "Lines", "Grid lines color", ColorSpec{0, 255, 128, 255}}
        }),
        [get_procedural_params](const ParameterBag& bag) {
    ...
    register_spec({
        ...
        ParameterSchema({
            {"motion_speed", "Speed", "Camera motion speed", 1.0, 0.0, 20.0},
            {"seed", "Seed", "Random seed", 0}
        }),
        [get_procedural_params](const ParameterBag& bag) {
    ...
    register_spec({
        ...
        ParameterSchema({
            {"motion_speed", "Speed", "Rotation speed", 1.0, 0.0, 10.0},
            {"mouse_influence", "Interaction", "Mouse repulsion strength", 2.0, 0.0, 10.0},
            {"mouse_radius", "Radius", "Mouse repulsion radius", 0.5, 0.0, 2.0}
        }),
        [get_procedural_params](const ParameterBag& bag) {
            auto spec = make_galaxy_spec(get_procedural_params(bag));
            spec.mouse_influence = bag.get_or<double>("mouse_influence", 2.0);
            spec.mouse_radius = bag.get_or<double>("mouse_radius", 0.5);
            spec.mouse_x = bag.get_or<float>("mouse_x", 0.5f);
            spec.mouse_y = bag.get_or<float>("mouse_y", 0.5f);
            return spec;
        }
    });
    ...
    register_spec({
        ...
        ParameterSchema({
            {"shape", "Shape", "Grid cell shape (square, circle, dot)", "square"},
            {"spacing", "Spacing", "Cell spacing", 40.0, 4.0, 200.0},
            {"border_width", "Width", "Line/Border width", 1.0, 0.1, 20.0}
        }),
        [get_procedural_params](const ParameterBag& bag) {

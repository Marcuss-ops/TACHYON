...
ShapePath ShapeModifiers::twist(const ShapePath& path, double angle, const Point2D& center) {
...
ShapePath ShapeModifiers::oscillator(const ShapePath& path, double frequency, double amplitude, double phase, double time, int axis) {
...
ShapePath ShapeModifiers::noise_deform(const ShapePath& path, double noise_scale, double amplitude, double time, uint64_t seed) {
    if (path.empty() || amplitude == 0.0) return path;

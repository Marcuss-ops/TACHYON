...
template <typename PropT, typename ValT, typename KfT>
void parse_property_body(
    const json& value,
    PropT& property,
    const std::string& path,
    DiagnosticBag& diagnostics,
    const char* key,
    std::function<bool(const json&, ValT&)> parse_val,
    std::function<bool(const json&, KfT&, const std::string&, DiagnosticBag&)> parse_kf
) {
    ValT v;
    if (parse_val(value, v)) { property.value = v; return; }
    if (value.is_object()) {
        if (value.contains("value") && parse_val(value.at("value"), v)) { property.value = v; return; }
        if (value.contains("keyframes") && value.at("keyframes").is_array()) {
            for (std::size_t i = 0; i < value.at("keyframes").size(); ++i) {
                KfT k;
                if (parse_kf(value.at("keyframes")[i], k,
                             make_path(path, key) + ".keyframes[" + std::to_string(i) + "]",
                             diagnostics))
...
void parse_optional_scalar_property(const json& object, const char* key, AnimatedScalarSpec& property, const std::string& path, DiagnosticBag& diagnostics) {
    ...
    // audio_band is only for scalars — handle it before the common body
    if (value.is_object() && value.contains("audio_band")) {
    ...
    parse_property_body<AnimatedScalarSpec, double, ScalarKeyframeSpec>(
        value, property, path, diagnostics, key,
        [](const json& j, double& v) { if (!j.is_number()) return false; v = j.get<double>(); return true; },
        parse_scalar_keyframe
    );
}
...
void parse_optional_vector_property(const json& object, const char* key, AnimatedVector2Spec& property, const std::string& path, DiagnosticBag& diagnostics) {
    if (!object.contains(key) || object.at(key).is_null()) return;
    parse_property_body<AnimatedVector2Spec, math::Vector2, Vector2KeyframeSpec>(
        object.at(key), property, path, diagnostics, key,
        parse_vector2_value, parse_vector2_keyframe
    );
}
...
void parse_optional_vector3_property(const json& object, const char* key, AnimatedVector3Spec& property, const std::string& path, DiagnosticBag& diagnostics) {
    if (!object.contains(key) || object.at(key).is_null()) return;
    parse_property_body<AnimatedVector3Spec, math::Vector3, AnimatedVector3Spec::Keyframe>(
        object.at(key), property, path, diagnostics, key,
        parse_vector3_value, parse_vector3_keyframe
    );
}
...
void parse_optional_color_property(const json& object, const char* key, AnimatedColorSpec& property, const std::string& path, DiagnosticBag& diagnostics) {
    if (!object.contains(key) || object.at(key).is_null()) return;
    parse_property_body<AnimatedColorSpec, ColorSpec, ColorKeyframeSpec>(
        object.at(key), property, path, diagnostics, key,
        parse_color_value, parse_color_keyframe
    );
}

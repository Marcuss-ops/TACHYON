...
std::uint64_t hash_scene_spec(const SceneSpec& scene, const DeterminismContract& contract) {
    ...
    for (const auto& composition : scene.compositions) {
        ...
        if (composition.background.has_value()) add_string(builder, *composition.background);
        ...
        for (const auto& layer : composition.layers) {
            ...
            for (const auto& animator : layer.text_animators) add_json(builder, animator);
            builder.add_u64(static_cast<std::uint64_t>(layer.text_highlights.size()));
            for (const auto& highlight : layer.text_highlights) add_json(builder, highlight);

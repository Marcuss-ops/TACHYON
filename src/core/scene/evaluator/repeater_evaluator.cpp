...
void evaluate_repeater_expansion(
    ...
    const double rep_count = sample_scalar(layer_spec.repeater.count, 1.0, remapped_time, context.audio_analyzer, hash_combine(layer_seed, stable_string_hash("repeater_count")), context.vars.numeric);
    ...
    const double stagger_delay = sample_scalar(layer_spec.repeater.stagger_delay, 0.0, remapped_time, context.audio_analyzer, hash_combine(layer_seed, stable_string_hash("stagger_delay")));
    const float off_x = static_cast<float>(sample_scalar(layer_spec.repeater.offset_position_x, 0.0, remapped_time, context.audio_analyzer, hash_combine(layer_seed, stable_string_hash("rep_off_x"))));
    const float off_y = static_cast<float>(sample_scalar(layer_spec.repeater.offset_position_y, 0.0, remapped_time, context.audio_analyzer, hash_combine(layer_seed, stable_string_hash("rep_off_y"))));
    const float off_rot = static_cast<float>(sample_scalar(layer_spec.repeater.offset_rotation, 0.0, remapped_time, context.audio_analyzer, hash_combine(layer_seed, stable_string_hash("rep_off_rot"))));
    const float off_sx = static_cast<float>(sample_scalar(layer_spec.repeater.offset_scale_x, 100.0, remapped_time, context.audio_analyzer, hash_combine(layer_seed, stable_string_hash("rep_off_sx")))) / 100.0f;
    const float off_sy = static_cast<float>(sample_scalar(layer_spec.repeater.offset_scale_y, 100.0, remapped_time, context.audio_analyzer, hash_combine(layer_seed, stable_string_hash("rep_off_sy")))) / 100.0f;
    const float start_op = static_cast<float>(sample_scalar(layer_spec.repeater.start_opacity, 100.0, remapped_time, context.audio_analyzer, hash_combine(layer_seed, stable_string_hash("rep_op_start")))) / 100.0f;
    const float end_op = static_cast<float>(sample_scalar(layer_spec.repeater.end_opacity, 100.0, remapped_time, context.audio_analyzer, hash_combine(layer_seed, stable_string_hash("rep_op_end")))) / 100.0f;
    ...
    for (int r = 0; r < iterations; ++r) {
        EvaluatedLayerState repeated = (stagger_delay != 0.0) 
            ? make_layer_state(context, layer_spec, layer_index, static_cast<double>(r) * stagger_delay, context.vars)
            : base_layer;

#include "tachyon/core/expressions/expression_vm.h"
#include "tachyon/core/expressions/expression_engine.h"
#include <vector>
#include <cmath>
#include <algorithm>
#include <stdexcept>

namespace tachyon::expressions {

double ExpressionVM::execute(const Bytecode& code, const ExpressionContext& context) {
    std::vector<double> stack;
    stack.reserve(64);

    for (const auto& instr : code.instructions) {
        switch (instr.op) {
            case OpCode::PushConst:
                if (instr.data >= code.constants.size()) throw std::runtime_error("VM Constant index out of bounds");
                stack.push_back(code.constants[instr.data]);
                break;
            case OpCode::PushVar: {
                if (instr.data >= code.names.size()) throw std::runtime_error("VM Name index out of bounds");
                const std::string& name = code.names[instr.data];
                auto it = context.variables.find(name);
                stack.push_back(it != context.variables.end() ? it->second : 0.0);
                break;
            }
            case OpCode::Add: {
                if (stack.size() < 2) throw std::runtime_error("VM Stack underflow on Add");
                double b = stack.back(); stack.pop_back();
                double a = stack.back(); stack.pop_back();
                stack.push_back(a + b);
                break;
            }
            case OpCode::Sub: {
                if (stack.size() < 2) throw std::runtime_error("VM Stack underflow on Sub");
                double b = stack.back(); stack.pop_back();
                double a = stack.back(); stack.pop_back();
                stack.push_back(a - b);
                break;
            }
            case OpCode::Mul: {
                if (stack.size() < 2) throw std::runtime_error("VM Stack underflow on Mul");
                double b = stack.back(); stack.pop_back();
                double a = stack.back(); stack.pop_back();
                stack.push_back(a * b);
                break;
            }
            case OpCode::Div: {
                if (stack.size() < 2) throw std::runtime_error("VM Stack underflow on Div");
                double b = stack.back(); stack.pop_back();
                double a = stack.back(); stack.pop_back();
                stack.push_back(b != 0.0 ? a / b : 0.0);
                break;
            }
            case OpCode::Pow: {
                if (stack.size() < 2) throw std::runtime_error("VM Stack underflow on Pow");
                double b = stack.back(); stack.pop_back();
                double a = stack.back(); stack.pop_back();
                stack.push_back(std::pow(a, b));
                break;
            }
            case OpCode::Neg: {
                if (stack.empty()) throw std::runtime_error("VM Stack underflow on Neg");
                stack.back() = -stack.back();
                break;
            }
            case OpCode::Call: {
                std::uint32_t name_idx = instr.data & 0xFFFF;
                std::uint32_t arg_count = instr.data >> 16;
                
                if (name_idx >= code.names.size()) throw std::runtime_error("VM Function name index out of bounds");
                const std::string& name = code.names[name_idx];
                
                if (stack.size() < arg_count) throw std::runtime_error("VM Stack underflow on Call " + name);

                std::vector<double> args(arg_count);
                for (int i = static_cast<int>(arg_count) - 1; i >= 0; --i) {
                    args[i] = stack.back();
                    stack.pop_back();
                }

                auto check_args = [&](std::uint32_t expected) {
                    if (arg_count != expected) throw std::runtime_error("Function " + name + " expects " + std::to_string(expected) + " arguments, got " + std::to_string(arg_count));
                };

                if (name == "sin") { check_args(1); stack.push_back(std::sin(args[0])); }
                else if (name == "cos") { check_args(1); stack.push_back(std::cos(args[0])); }
                else if (name == "abs") { check_args(1); stack.push_back(std::abs(args[0])); }
                else if (name == "min") { check_args(2); stack.push_back(std::min(args[0], args[1])); }
                else if (name == "max") { check_args(2); stack.push_back(std::max(args[0], args[1])); }
                else if (name == "clamp") { check_args(3); stack.push_back(std::clamp(args[0], args[1], args[2])); }
                else if (name == "wiggle") {
                    check_args(5);
                    stack.push_back(std::sin(args[0] * args[4]) * args[1]); 
                }
                else if (name == "pingpong") {
                    check_args(2);
                    double t = args[0];
                    double length = args[1];
                    double normalized = std::fmod(t, 2.0 * length);
                    if (normalized > length) normalized = 2.0 * length - normalized;
                    stack.push_back(normalized);
                }
                else if (name == "timeStretch") {
                    check_args(2);
                    double t = args[0];
                    double factor = args[1];
                    stack.push_back(t * factor);
                }
                else if (name == "stagger") {
                    check_args(1);
                    double base_time = args[0];
                    auto it = context.variables.find("index");
                    double index = (it != context.variables.end()) ? it->second : 0.0;
                    stack.push_back(base_time * index);
                }
                else if (name == "lerp") {
                    check_args(3);
                    double a = args[0];
                    double b = args[1];
                    double t = args[2];
                    stack.push_back(a + (b - a) * t);
                }
                else if (name == "interpolate") {
                    check_args(3);
                    double a = args[0];
                    double b = args[1];
                    double t = args[2];
                    stack.push_back(a + (b - a) * t);
                }
                else if (name == "spring") {
                    check_args(5);
                    double t = args[0];
                    double from = args[1];
                    double to = args[2];
                    double freq = args[3];
                    double damping = args[4];
                    double omega = 2.0 * 3.14159 * freq;
                    double decay = std::exp(-damping * t);
                    stack.push_back(from + (to - from) * (1.0 - decay * std::cos(omega * t)));
                }
                else if (name == "radiansToDegrees") {
                    check_args(1);
                    stack.push_back(args[0] * 57.29577951308232);
                }
                else if (name == "degreesToRadians") {
                    check_args(1);
                    stack.push_back(args[0] * 0.017453292519943295);
                }
                else {
                    stack.push_back(0.0);
                }
                break;
            }
            case OpCode::Ret:
                return stack.empty() ? 0.0 : stack.back();
        }
    }
    return stack.empty() ? 0.0 : stack.back();
}

} // namespace tachyon::expressions

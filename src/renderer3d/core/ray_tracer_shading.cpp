    ...
    int depth) 
{
    ...
    // Evaluate full material sample
    MaterialSample mat_sample = MaterialEvaluator::evaluate(*layer, hit_pos, normal, uv);

    if (sub_mesh) {
        mat_sample.base_color = mat_sample.base_color * sub_mesh->material.base_color_factor;
        mat_sample.metallic = mat_sample.metallic * sub_mesh->material.metallic_factor;
        mat_sample.roughness = std::max(mat_sample.roughness * sub_mesh->material.roughness_factor, 0.05f);
        if (sub_mesh->material.base_color_texture_idx >= 0 && parent_asset && sub_mesh->material.base_color_texture_idx < (int)parent_asset->textures.size()) {
            ...
            if (!tex.data.empty() && tex.width > 0 && tex.height > 0 && tex.channels > 0) {
                ...
                mat_sample.base_color = mat_sample.base_color * math::Vector3{ tex.data[idx+0]/255.0f, tex.data[idx+1]/255.0f, tex.data[idx+2]/255.0f };
            }
        }
        if (sub_mesh->material.metallic_roughness_texture_idx >= 0 && parent_asset && sub_mesh->material.metallic_roughness_texture_idx < (int)parent_asset->textures.size()) {
            ...
            if (!tex.data.empty() && tex.width > 0 && tex.height > 0 && tex.channels >= 3) {
                ...
                int ty = std::clamp((int)((uv.y - std::floor(uv.y)) * tex.height), 0, tex.height - 1);
                int idx = (ty * tex.width + tx) * tex.channels;
                mat_sample.roughness = std::max(mat_sample.roughness * (tex.data[idx+1]/255.0f), 0.05f); 
                mat_sample.metallic = mat_sample.metallic * (tex.data[idx+2]/255.0f);
            }
        }
    }
    
    math::Vector3 albedo = mat_sample.base_color;
    float metallic = mat_sample.metallic;
    float roughness = std::max(mat_sample.roughness, 0.05f);
    ...
    for (const auto& light : state.lights) {
        ...
        if (light.type == "ambient") { total_lighting += mat_sample.base_color * light.color.to_vector3() * light.intensity * 0.1f; continue; }
        ...
        if (!in_shadow) {
            ...
            math::Vector3 bsdf = evaluate_disney_bsdf(mat_sample, normal, view_vec, l_dir);
            float nl = std::clamp(math::Vector3::dot(normal, l_dir), 0.0f, 1.0f);
            total_lighting += bsdf * light_rgb * (light.intensity * attenuation) * nl;
        }
    }
    ...
    if (depth < kMaxBounces) {
        ...
        if (trans > 0.01f) {
            float eta = 1.0f / mat_sample.ior; math::Vector3 n = normal; if (math::Vector3::dot(direction, n) > 0) { n = -normal; eta = mat_sample.ior; }
    ...
    total_lighting += mat_sample.emissive;

...
void RayTracer::render(const scene::EvaluatedCompositionState& state, float* out_rgba, float* out_depth, int width, int height) {
    render(state, out_rgba, out_depth, width, height, nullptr, 0.0, 0.0);
}
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            ...
            for (std::size_t bt = 0; bt < blur_times.size(); ++bt) {
                ...
                std::mt19937 rng(static_cast<unsigned int>(state.frame_number + y * width + x + static_cast<int>(sample_time * 1000.0)));
                ...
                for (int s = 0; s < spp; ++s) {
                    ...
                    if (s == 0 && bt == 0) { f_alb = result.albedo; f_nor = result.normal; }
                }
            }
            int idx = (y * width + x) * 4;
            const float denom = static_cast<float>(spp) * blur_weight_sum;
            out_rgba[idx+0] = acc_rgb.x / denom; out_rgba[idx+1] = acc_rgb.y / denom; out_rgba[idx+2] = acc_rgb.z / denom; out_rgba[idx+3] = std::clamp(acc_alpha / denom, 0.0f, 1.0f);
            if (out_depth) out_depth[y * width + x] = acc_depth / denom;
            if (!albedo_aux.empty()) {

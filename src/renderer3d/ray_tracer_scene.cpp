...
void RayTracer::build_scene(const scene::EvaluatedCompositionState& state, const text::Font* font) {
    ...
    environment_intensity_ = state.environment_intensity; environment_rotation_ = state.environment_rotation;
    environment_manager_.ensure_brdf_lut();
    internal_build_scene(state, [](std::size_t) { return true; }, font);
}
...
void RayTracer::build_scene_subset(const scene::EvaluatedCompositionState& state, const std::vector<std::size_t>& layer_indices, const text::Font* font) {
    ...
    environment_intensity_ = state.environment_intensity; environment_rotation_ = state.environment_rotation;
    environment_manager_.ensure_brdf_lut();
    internal_build_scene(state, [&](std::size_t idx) { return std::find(layer_indices.begin(), layer_indices.end(), idx) != layer_indices.end(); }, font);
}
...
void RayTracer::internal_build_scene(const scene::EvaluatedCompositionState& state, const std::function<bool(std::size_t)>& filter, const text::Font* font) {
    for (std::size_t layer_idx = 0; layer_idx < state.layers.size(); ++layer_idx) {
        ...
        if (media_mesh && !media_mesh->empty()) {
        ...
        } else if (layer.type == scene::LayerType::Text && !layer.text_content.empty() && font) {
            text::TextStyle style; style.pixel_size = (uint32_t)layer.font_size;
            text::TextBox box{ 10000, 10000, true }; text::TextAlignment align = (layer.text_alignment == 1) ? text::TextAlignment::Center : (layer.text_alignment == 2 ? text::TextAlignment::Right : text::TextAlignment::Left);
            ...
            const std::uint32_t pk = 0xFFFFFFFF; auto it = mesh_cache_.find(pk); if (it != mesh_cache_.end()) sub_scene = it->second.scene;
                ...
                v[0]=-0.5f;v[1]=-0.5f;v[2]=0;v[3]=0.5f;v[4]=-0.5f;v[5]=0;v[6]=0.5f;v[7]=0.5f;v[8]=0;v[9]=-0.5f;v[10]=0.5f;v[11]=0;
            ...
            RTCGeometry inst = rtcNewGeometry(device_, RTC_GEOMETRY_TYPE_INSTANCE); rtcSetGeometryInstancedScene(inst, sub_scene);
            math::Matrix4x4 m = layer.world_matrix * math::Matrix4x4::scaling({(float)layer.width, (float)layer.height, 1.0f});

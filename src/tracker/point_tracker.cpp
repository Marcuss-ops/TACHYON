#include "tachyon/tracker/point_tracker.h"
...
TrackBinding PointTracker::create_binding(
    ...
    TrackBinding binding(track, target);
    // Store layer_id in user data if needed
    return binding;
}
    ...
    m_bindings.erase(
        std::remove_if(m_bindings.begin(), m_bindings.end(),
            [&](const TrackBinding& b) {
                return b.track().name == track_id && /* layer check */ true;
            }),
        m_bindings.end());
}

...
extern "C" __declspec(dllexport) void build_scene(tachyon::SceneSpec& scene) {
    ...
    // --- SCENE B (Front, Transitions In) ---
    {
        ...
        l.transition_in.transition_id = "tachyon.transition.slide_up";
        l.transition_in.type = "tachyon.transition.slide_up";
    ...
    comp.layer("text_b", [&](LayerBuilder& l) {
        ...
        l.enter().id("tachyon.transition.slide_up").duration(1.0).done();
    });

    // Header Label
    comp.layer("label", [&](LayerBuilder& l) {
        l.text().content("SLIDE_UP").font("SFPro").font_size(80).box(1920, 150).align(HorizontalAlign::Center).done()

...
extern "C" __declspec(dllexport) void build_scene(tachyon::SceneSpec& scene) {
    ...
    // --- SCENE B (Front, Transitions In) ---
    {
        ...
        l.transition_in.transition_id = std::string(ids::transition::glitch_slice);
        l.transition_in.type = std::string(ids::transition::glitch_slice);
    ...
    comp.layer("text_b", [&](LayerBuilder& l) {
        ...
        l.enter().id(std::string(ids::transition::glitch_slice)).duration(1.0).done();
    });

    // Header Label
    comp.layer("label", [&](LayerBuilder& l) {
        l.text().content("GLITCH_SLICE").font("SFPro").font_size(80).box(1920, 150).align(HorizontalAlign::Center).done()

...
extern "C" __declspec(dllexport) void build_scene(tachyon::SceneSpec& scene) {
    ...
    // --- SCENE B (Front, Transitions In) ---
    {
        ...
        l.transition_in.transition_id = "tachyon.transition.wipe_linear";
        l.transition_in.type = "tachyon.transition.wipe_linear";
    ...
    comp.layer("text_b", [&](LayerBuilder& l) {
        ...
        l.enter().id("tachyon.transition.wipe_linear").duration(1.0).done();
    });

    // Header Label
    comp.layer("label", [&](LayerBuilder& l) {
        l.text().content("WIPE_LINEAR").font("SFPro").font_size(80).box(1920, 150).align(HorizontalAlign::Center).done()

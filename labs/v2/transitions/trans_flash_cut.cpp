...
extern "C" __declspec(dllexport) void build_scene(tachyon::SceneSpec& scene) {
    ...
    // --- SCENE B (Front, Transitions In) ---
    {
        ...
        l.transition_in.transition_id = "tachyon.transition.flash_cut";
        l.transition_in.type = "tachyon.transition.flash_cut";
    ...
    comp.layer("text_b", [&](LayerBuilder& l) {
        ...
        l.enter().id("tachyon.transition.flash_cut").duration(1.0).done();
    });

    // Header Label
    comp.layer("label", [&](LayerBuilder& l) {
        l.text().content("FLASH_CUT").font("SFPro").font_size(80).box(1920, 150).align(HorizontalAlign::Center).done()

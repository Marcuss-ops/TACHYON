...
extern "C" __declspec(dllexport) void build_scene(tachyon::SceneSpec& scene) {
    ...
    // --- SCENE B (Front, Transitions In) ---
    {
        ...
        l.transition_in.transition_id = std::string(ids::transition::light_leak);
        l.transition_in.type = std::string(ids::transition::light_leak);
    ...
    comp.layer("text_b", [&](LayerBuilder& l) {
        ...
        l.enter().id(std::string(ids::transition::light_leak)).duration(1.0).done();
    });

    // Header Label
    comp.layer("label", [&](LayerBuilder& l) {
        l.text().content("LIGHT_LEAK").font("SFPro").font_size(80).box(1920, 150).align(HorizontalAlign::Center).done()

...
extern "C" tachyon::SceneSpec build_scene() {
    ...
    auto main_comp = Composition("main", effects).size(1920, 1080).duration(5.0).fps(30)
        ...
        .layer("text_layer", [](LayerBuilder& l) {
            ...
            anim.name = "horizontal_slide";
            anim.selector.type = "all";
            anim.selector.based_on = "characters";
            anim.selector.stagger_delay = 0.04;
            
            anim.properties.opacity_keyframes.push_back({0.0, 0.0});
            anim.properties.opacity_keyframes.push_back({0.4, 1.0});
            
            anim.properties.position_offset_keyframes.push_back({0.0, {-30.0, 0.0}});
            anim.properties.position_offset_keyframes.push_back({0.4, {0.0, 0.0}});

            l.text().content("MINIMAL SLIDE IN").font("SFPro").font_size(120)

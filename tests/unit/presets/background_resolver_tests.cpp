...
bool run_background_resolver_tests() {
    ...
    // 2. Color without parseable value → empty
    {
        BackgroundSpec bg;
        bg.type = BackgroundType::Color;
        // parsed_color left empty — resolver should produce nothing
        auto layers = resolve_background_to_layers(bg, 1920, 1080);
        assert(layers.empty());
    }
    ...
    // 3. Asset → single Image layer with asset_id set
    {
        ...
        auto layers = resolve_background_to_layers(bg, 1280, 720, 3.0);
        assert(layers.size() == 1);
    ...
    // 4. Preset (valid) → single non-null layer with preset_id set
    {
        ...
        auto layers = resolve_background_to_layers(bg, 1920, 1080, 8.0);
        assert(layers.size() == 1);
    ...
    // 5. Preset (non-existent) → empty — resolver must not crash or return nulls
    {
        BackgroundSpec bg;
        bg.type = BackgroundType::Preset;
    ...
    // 6. Component → empty (unsupported, must be silent no-op, not a crash)
    {
        BackgroundSpec bg;

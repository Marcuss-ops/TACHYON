...
bool run_scene_evaluator_tests() {
    ...
    check_true(nearly_equal(evaluated->time_seconds, 1.5), "frame 45 at 30 fps resolves to 1.5 seconds");
    ...
    check_true(nearly_equal(evaluated->layers[1].transform2.position.x, 50.0), "linear keyframe position.x resolves halfway");
    check_true(nearly_equal(evaluated->layers[1].transform2.position.y, 25.0), "linear keyframe position.y resolves halfway");

    check_true(evaluated->camera.enabled, "camera layer resolves an enabled camera state");
    ...
    check_true(nearly_equal(evaluated->camera.projection[11], -1.0), "camera projection uses perspective clip convention");

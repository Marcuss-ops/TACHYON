...
protected:
    ...
    void run_golden_test(const std::string& name, const std::string& scene_json) {
        ...
        plan.render_plan.composition.width = 160;  // Small for speed
        plan.render_plan.composition.height = 90;
        plan.render_plan.composition.frame_rate = 24.0;
        ...
        // 5. Check against expected (this would be in a JSON file normally)
        // For now, we print the checksum if it's a new test
        std::cout << "[GOLDEN] " << name << " Checksum: " << std::hex << checksum << std::dec << std::endl;
        
        // 6. Save image for manual verification if checksum mismatch (placeholder for real diff logic)
        // In a real studio system, we would compare against tests/golden/reference/<name>.png
    }
};

...
bool AudioDecoder::open(const std::filesystem::path& path) {
...
#if !defined(TACHYON_HAS_FFMPEG)
    tachyon::diag::error("[AudioDecoder] FFmpeg not found at compile time, cannot open {}", path.string());
    return false;
#else
    ...
    if (ret != 0) {
        ...
        tachyon::diag::error("[AudioDecoder] could not open {}: {}", path.string(), errbuf);
        return false;
    }

    if (avformat_find_stream_info(m_format_context, nullptr) < 0) {
        tachyon::diag::error("[AudioDecoder] could not find stream info for {}", path.string());
        return false;
    }

    m_stream_index = av_find_best_stream(m_format_context, AVMEDIA_TYPE_AUDIO, -1, -1, nullptr, 0);
    if (m_stream_index < 0) {
        tachyon::diag::error("[AudioDecoder] no audio stream in {}", path.string());
        return false;
    }
    ...
    if (!codec) {
        tachyon::diag::error("[AudioDecoder] decoder not found for codec {} in {}", static_cast<int>(stream->codecpar->codec_id), path.string());
        return false;
    }

    m_codec_context = avcodec_alloc_context3(codec);
    if (!m_codec_context) {
        tachyon::diag::error("[AudioDecoder] could not allocate codec context for {}", path.string());
        return false;
    }

    if (avcodec_parameters_to_context(m_codec_context, stream->codecpar) < 0) {
        tachyon::diag::error("[AudioDecoder] could not copy codec parameters for {}", path.string());
        return false;
    }

    if (avcodec_open2(m_codec_context, codec, nullptr) < 0) {
        tachyon::diag::error("[AudioDecoder] could not open codec for {}", path.string());
        return false;
    }
    ...
    if (!m_packet || !m_frame) {
        tachyon::diag::error("[AudioDecoder] could not allocate FFmpeg primitives for {}", path.string());
        return false;
    }
    return true;
#endif

        ...
        for (const auto& layer : comp.layers) {
            ...
            if (auto* media = std::get_if<MediaSource>(&layer.source)) {
                if (layer.identity.type == LayerType::Image) {
                    refs.push_back({media->asset.id, media->asset.id, AssetKind::Image, layer.identity.id});
                } else if (layer.identity.type == LayerType::Video) {
                    refs.push_back({media->asset.id, media->asset.id, AssetKind::Video, layer.identity.id});
                }
            }

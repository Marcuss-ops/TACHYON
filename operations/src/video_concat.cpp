...
core::MediaResult<void> VideoConcat::concat_videos(const ConcatConfig& config) {
    if (config.inputs.empty()) {
        return core::MediaResult<void>::failure(
            core::MediaError(core::MediaErrorCode::IO, "No input files provided for concatenation")
        );
    }
    ...
    if (!list_file.is_open()) {
        return core::MediaResult<void>::failure(
            core::MediaError(core::MediaErrorCode::IO, "Failed to create temporary concat list")
    ...
    for (const auto& input : config.inputs) {
        if (!std::filesystem::exists(input)) {
             return core::MediaResult<void>::failure(
                core::MediaError(core::MediaErrorCode::IO, "Input file missing")
    ...
    if (!result.success || result.exit_code != 0) {
        return core::MediaResult<void>::failure(
            core::MediaError(core::MediaErrorCode::Encode, "FFmpeg concatenation failed: " + result.error)

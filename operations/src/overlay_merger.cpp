#include "tachyon/media/overlay_merger.h"
...
core::MediaResult<void> OverlayMerger::merge_overlays(const OverlayMergeConfig& config) {
    ...
    if (!result.success || result.exit_code != 0) {
        return core::MediaResult<void>::failure(
            core::MediaError(core::MediaErrorCode::Encode, "FFmpeg overlay merge failed: " + result.error)
